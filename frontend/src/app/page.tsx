...
export default function Home() {
  ...
  const callLLMDirect = async () => {
    try {
      ...
      const llmTxHash = await ethereum.request({ method: "eth_sendTransaction", params: [{ from: address, to: LLM_PRECOMPILE, data: encoded, gas: "0x4C4B40" }] });
      log(`LLM TX: ${llmTxHash}`);

      // Poll for result
      log("Waiting for LLM settlement...");
      let result = "";
      for (let i = 0; i < 30; i++) {
        ...
        if (receipt?.status === "0x0") { throw new Error("LLM TX failed on-chain"); }
      }

      if (!result) throw new Error("No LLM result after 90s. Check RitualWallet deposit.");
      
      log(`LLM Result: ${result.slice(0, 100)}...`);
      setInterpretation(result);

      // Store result on-chain
      log("Storing result on-chain...");
      setStatus("storing");
      writeContract({ address: CONTRACT_ADDRESS, abi: CONTRACT_ABI, functionName: "storeResult", args: [BigInt(dreamId ?? 0), result] }, {
        onSuccess: () => { log("✅ Dream saved on-chain!"); setStatus("done"); },
        onError: (error) => { log(`Store error: ${error.message}`); setStatus("done"); },
      });
