...
export default function Home() {
  ...
  const handleDeposit = async () => {
    ...
    try {
      log("Depositing 0.5 RIT to RitualWallet FOR CONTRACT...");
      writeContract({
        address: RITUAL_WALLET,
        abi: [{ name: "depositFor", type: "function", stateMutability: "payable", inputs: [{ name: "account", type: "address" }, { name: "lockDuration", type: "uint256" }], outputs: [] }],
        functionName: "depositFor",
        args: [CONTRACT_ADDRESS, 100000n],
  ...
  const callLLMViaContract = async () => {
    try {
      ...
      writeContract({
        ...
        onSuccess: () => { log("✅ interpretDream TX sent! LLM processing on-chain..."); setStatus("storing"); pollForResult(); },
  ...
  const pollForResult = async () => {
    log("Polling for LLM result (max 90s)...");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const eth = (window as any).ethereum;
    ...
    for (let i = 0; i < 30; i++) {
      ...
      try {
        ...
        if (receipt?.status === "0x0") {
          log("TX failed on-chain!");
          setStatus("error"); setErrorMessage("LLM TX failed. Check RitualWallet deposit.");
          return;
        }
        if (receipt?.status === "0x1") {
          log("✅ TX confirmed! LLM result saved on-chain.");
          setStatus("done");
          return;
        }
      } catch {}
    }
    log("⚠️ Timeout. Check explorer.");
    setStatus("done");
  };
    ...
    <main className="flex-1 flex flex-col items-center justify-center p-4">
      ...
      {isConnected && !isWrongNetwork && (
        <div className="w-full max-w-md">
          <div className="mb-4"><button onClick={handleDeposit} disabled={status === "depositing"} className="w-full bg-gradient-to-r from-green-700 to-emerald-700 hover:from-green-600 hover:to-emerald-600 disabled:from-gray-700 disabled:text-gray-500 text-white py-2 rounded-lg text-sm font-medium">{status === "depositing" ? "⏳ Depositing..." : "💰 Deposit 0.5 RIT for Contract LLM Fees"}</button></div>
